...
export interface StatsComponentRef {
  forceUpdate: () => void;
  closeConfig: () => void;
  openConfig: (configObj: Entity | Trap) => void;
}

const StatsComponent = forwardRef<StatsComponentRef, Props>((props: Props, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    forceUpdate,
    closeConfig,
    openConfig: (configObj: Entity | Trap) => {
      openConfig(configObj);
    }
  }));

  const onSelectMap = (value: string) => {
    if (confirm("Attention, changer la map réinitialisera tous les pièges et entités !")) {
      Game.loadMap(value);
    }
  }

  const onReorderTraps = (oldPos: number, newPos: number) => {
  ...
  const onClickMainChara = () => {
    openConfig(Game.mainCharacter);
  }
  ...
  const openConfig = (obj: Entity | Trap) => {
    setConfigObj(obj);
  }
  ...
  if (configObj !== null && configObj instanceof Entity) {
  ...
  return <div className="relative-height stats">
    ...
    <select onChange={(e) => { onSelectMap(e.target.value); }}>
      {mapOptions}
    </select>
    <div className="share">
      <button className="shareBtn" onClick={() => { onImport(); }}><Trans>Import</Trans></button>
    ...
    <div className="main-character" onClick={() => { onClickMainChara(); }}><Trans>My character</Trans></div>
    <hr />
    <Reorder
      className="trap-list"
      reorderId="trap-list"
      ...
      lock="horizontal"
      touchHoldTime={500}
      onReorder={(_e: unknown, startIdx: number, endIdx: number) => { onReorderTraps(startIdx, endIdx); }}
      autoScroll={true}
    >
      {traps.map((trap, idx) => {
        return (
    ...
    <div className={`btn-order ${order ? 'active' : ''}`} onClick={() => { onOrderClick(); }}><Trans>Ordre de pose</Trans></div>
    {help}
  </div>;
});
